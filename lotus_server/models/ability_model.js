...
const abilityFuncs = {
  ...
  nanomachine_swarm: function(player){
    ...
    for(const monsterId in team){
      ...
      if(!curMonster.modifiers.has('heal') && curMonster.hp < curMonster.maxHp){
        ...
        const description = 'Monster is healed 200 hp per turn until fully healed.';
  ...
  electric_shield: function(player){
    ...
    const description = `${this.name} takes the first 500 damage ${targetMonster.name} takes each turn.`;
